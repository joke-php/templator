...
/**
 * Обработчик узлов блоков с непарными директивами.
 *
 * Выступает в роли диспетчера: определяет тип директивы (например, 'csrf')
...
class StatementNodeHandler implements NodeHandlerInterface
{
    ...
    /**
     * @var class-string Ожидаемый класс ноды
     */
    protected string $nodeClass = StatementNode::class;
     ...
     */
    protected function getErrorMessage(NodeInterface $node): string
    {
        return sprintf('Expected instance of %s, got %s.', $this->nodeClass, $node::class);
    }
}
